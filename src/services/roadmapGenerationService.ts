...
/**
 * Path to Cedar policies file relative to workspace root
 * Generated by Step 8 when security configuration includes compliance frameworks
 */
const CEDAR_POLICIES_PATH = 'policies/main.cedar';
...
export class RoadmapGenerationService implements vscode.Disposable {
  ...
  /**
   * Check if Cedar policies exist and optionally load their content
   * Cedar policies are generated by Step 8 when security configuration
   * includes compliance frameworks or approval gates.
   *
   * @returns Object with exists flag and optional content
   */
  public async checkCedarPoliciesExist(): Promise<{
    exists: boolean;
    content?: string;
  }> {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    ...
    const cedarUri = vscode.Uri.joinPath(workspaceFolder.uri, CEDAR_POLICIES_PATH);

    try {
      // Check if file exists
      await vscode.workspace.fs.stat(cedarUri);

      // Load the content
      const content = await vscode.workspace.fs.readFile(cedarUri);
      return {
        exists: true,
        content: Buffer.from(content).toString('utf-8'),
      };
   ...
   * 4. Check for Cedar policies in policies/main.cedar
   * 5. Load the roadmap prompt
   * 6. Call Bedrock API with prompt, XML context, and Cedar policy info
   * 7. Fire onGenerationComplete or onGenerationError event
  ...
  public async generateRoadmap(): Promise<string> {
    ...
    try {
      ...
      // Check for Cedar policies (generated by Step 8 from security configuration)
      const cedarPolicies = await this.checkCedarPoliciesExist();

      // Build Cedar policy context if policies exist
      let cedarContext = '';
      if (cedarPolicies.exists && cedarPolicies.content) {
        cedarContext = `\n\n<cedar_policies_exist>true</cedar_policies_exist>\n\n<cedar_policies>\n${cedarPolicies.content}\n</cedar_policies>`;
      } else {
        cedarContext = '\n\n<cedar_policies_exist>false</cedar_policies_exist>';
      }
      ...
      // Build user message with XML-formatted steering files and Cedar policy context
      const userMessage = `Generate the implementation roadmap based on the following steering files:\n\n${xmlContext}${cedarContext}`;
