...
<cedar_policies_exist>true|false</cedar_policies_exist>

<cedar_policies>
{content of policies/main.cedar if exists}
</cedar_policies>
```

**Cedar Policy Detection:** Check the `<cedar_policies_exist>` tag. If `true`, Cedar policies exist and you MUST include the Cedar Policies section in the roadmap.
...
**CONDITIONAL:** This section is ONLY included if Cedar policies exist. Check the `<cedar_policies_exist>` XML tag in the input context — if it contains `true`, include this section and use the content from `<cedar_policies>` tag.
...
33. [ ] IF `<cedar_policies_exist>` is `true`, include Cedar Policies section in roadmap
34. [ ] IF `<cedar_policies_exist>` is `true`, include note in "How to Use This Roadmap" section
35. [ ] Policy Engine setup is documented as handled by `setup.sh`, NOT a Kiro item
36. [ ] Do NOT generate a separate roadmap item for Policy Engine creation
37. [ ] If Cedar policies exist, explain their purpose (access control from Step 4 security inputs)
