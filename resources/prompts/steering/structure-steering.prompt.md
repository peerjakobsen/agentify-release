...
**IMPORTANT:** The following files are **pre-bundled** by the Agentify extension during project initialization:
- `agents/shared/` — Shared utilities (IMPORT from, do not recreate)
- `agents/main.py` — Pattern-specific orchestrator (MODIFY CUSTOMIZATION SECTION only, do not recreate)

```
project-root/
├── agents/
│   ├── main.py                    # PRE-BUNDLED orchestrator (modify CUSTOMIZATION SECTION only)
...
The root `pyproject.toml` contains agent runtime dependencies. It is created by the Agentify extension during project initialization.

**CRITICAL**: `bedrock-agentcore-starter-toolkit` MUST be in `[project.optional-dependencies].dev`, NOT in main `[project].dependencies`:

```toml
[project]
dependencies = [
    "strands-agents>=0.1.0",      # Agent runtime
    "bedrock-agentcore>=1.0.0",   # AgentCore runtime library
    "mcp>=1.0.0",                 # MCP client for Gateway
...
dev = [
    "pytest>=7.0.0",
    "bedrock-agentcore-starter-toolkit>=0.2.0",  # CLI only - NOT at runtime!
]
```

**Why this matters**: The `bedrock-agentcore-starter-toolkit` requires GCC to build (via `ruamel-yaml-clibz` dependency). The slim Docker image used for agent deployment doesn't have GCC, causing build failures if the toolkit is in main dependencies. It's only needed for CLI commands (`agentcore configure/deploy`), not at agent runtime.
...
| `uv sync --all-extras` | Install main + dev dependencies |
| `uv run agentcore deploy` | Deploy agents (requires dev deps locally) |
| `uv run python agents/main.py` | Run workflow locally |

### Dependency Files

| File | Purpose | Modify? |
|------|---------|---------|
| `pyproject.toml` | Dependency declarations | Yes - add new deps here |
| `uv.lock` | Locked versions (auto-generated) | No - regenerated by uv |
| `cdk/pyproject.toml` | CDK-specific deps (separate venv) | Rarely |
...
**CRITICAL Handler Pattern - Docker Host Binding**: Handlers must detect Docker environment and bind to `0.0.0.0` for external access:
...
if __name__ == '__main__':
    ...
    # CRITICAL: Detect Docker and bind to 0.0.0.0 for external access
    # Without this, container binds to localhost and health checks fail
    is_docker = os.environ.get('DOCKER_CONTAINER') == '1'
    host = '0.0.0.0' if is_docker else None

    logger.info(f'Starting {agent_name} on port {port}, host={host or "default"}')
