...
# Step 2b: Setup AgentCore Policy Engine (if Cedar policies exist)
...
# Check if policies directory exists and has .cedar files
CEDAR_FILES=$(find "$POLICIES_DIR" -name "*.cedar" 2>/dev/null | head -1)

if [ -n "$CEDAR_FILES" ] && [ -f "$GATEWAY_CONFIG" ]; then
    cd "${CDK_DIR}"
    ...
    if [ -n "$GATEWAY_ARN" ]; then
        ...
        if [ -z "$EXISTING_POLICY_ENGINE_ID" ]; then
            ...
            print_step "Creating Policy Engine: ${PROJECT_NAME}-policy-engine..."
            ...
            # Output format: "Policy Engine ID: testPolicyEngine-abc123"
            POLICY_ENGINE_ID=$(echo "$POLICY_ENGINE_OUTPUT" | grep -i "policy.engine" | grep -oE '[a-zA-Z0-9_-]+-[a-zA-Z0-9]+$' | head -1 || true)
            
            # Alternative extraction if above fails
            if [ -z "$POLICY_ENGINE_ID" ]; then
                POLICY_ENGINE_ID=$(echo "$POLICY_ENGINE_OUTPUT" | grep -oE "${PROJECT_NAME}-policy-engine-[a-zA-Z0-9]+" | head -1 || true)
            fi
            ...
            if [ -n "$POLICY_ENGINE_ID" ]; then
                ...
                # Try to find the policy engine by listing
                POLICY_ENGINE_ID=$(uv run agentcore policy list-policy-engines --region "${REGION}" 2>&1 | \
                    grep -i "${PROJECT_NAME}" | grep -oE '[a-zA-Z0-9_-]+-[a-zA-Z0-9]+' | head -1 || true)
        ...
        if [ -n "$POLICY_ENGINE_ID" ]; then
            ...
            POLICY_COUNT=0
            for CEDAR_FILE in "$POLICIES_DIR"/*.cedar; do
                if [ -f "$CEDAR_FILE" ]; then
                    POLICY_NAME=$(basename "$CEDAR_FILE" .cedar)
                    # Convert hyphens to underscores (Cedar policy names must match pattern ^[A-Za-z][A-Za-z0-9_]*$)
                    POLICY_NAME_SANITIZED=$(echo "$POLICY_NAME" | sed 's/-/_/g')
                    
                    # Read Cedar content and replace Gateway ARN placeholder
                    CEDAR_CONTENT=$(cat "$CEDAR_FILE")
                    CEDAR_CONTENT=$(echo "$CEDAR_CONTENT" | sed "s|{{GATEWAY_ARN}}|$GATEWAY_ARN|g")
                    
                    # Escape for JSON (handle newlines and special characters)
                    CEDAR_ESCAPED=$(echo "$CEDAR_CONTENT" | jq -Rs .)
                    # Remove surrounding quotes that jq adds
                    CEDAR_ESCAPED="${CEDAR_ESCAPED:1:-1}"

                    print_step "Creating policy: $POLICY_NAME_SANITIZED"
                    
                    # Create the policy
                    POLICY_DEFINITION="{\"cedar\":{\"statement\":\"$CEDAR_ESCAPED\"}}"
                    
                    if uv run agentcore policy create-policy \
                        --policy-engine-id "$POLICY_ENGINE_ID" \
                        --name "$POLICY_NAME_SANITIZED" \
                        --description "Generated from $CEDAR_FILE" \
                        --definition "$POLICY_DEFINITION" \
                        --region "${REGION}" 2>/dev/null; then
                        print_success "Created policy: $POLICY_NAME_SANITIZED"
                        POLICY_COUNT=$((POLICY_COUNT + 1))
                    else
                        print_warning "Failed to create policy $POLICY_NAME_SANITIZED (may already exist)"
                    fi
                fi
            done

            if [ $POLICY_COUNT -gt 0 ]; then
                print_success "Created $POLICY_COUNT Cedar policies"
            fi
            ...
            if [ -f "$INFRA_CONFIG" ] && command -v jq &> /dev/null; then
                jq --arg peid "$POLICY_ENGINE_ID" \
                   ...
                   --arg mode "$POLICY_MODE" \
                    '.policy = {"policyEngineId": $peid, "policyEngineArn": $pearn, "mode": $mode}' \
    ...
    if [ -z "$CEDAR_FILES" ]; then
        print_warning "No policies/ directory or .cedar files found"
    fi
