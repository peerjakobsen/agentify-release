...
**New Prompt File:**
`resources/prompts/steering/cedar-policies.prompt.md` — Transforms wizard state into Cedar policy syntax:
...
- `data-access.cedar` — Data sensitivity controls
- `approval-gates.cedar` — Human approval requirements  
- `compliance/{framework}.cedar` — Framework-specific policies (HIPAA, PCI-DSS, etc.)
```

**Generated File Structure:**
```
policies/
├── main.cedar              # Base permit/deny rules
├── data-access.cedar       # Sensitivity-based access controls
├── approval-gates.cedar    # Human approval requirements
└── compliance/
    ├── hipaa.cedar         # If HIPAA selected
    ├── pci-dss.cedar       # If PCI-DSS selected
    └── ...                 # Other frameworks
```
...
**setup.sh Integration (Step 2b — after Gateway, before Agent):**
```bash
...
**destroy.sh Integration (Step 1b — before Gateway cleanup):**
```bash
...
```
14:32:02  🛡️ Policy: ALLOW get_customer (data-access.cedar)
14:32:03  🛡️ Policy: DENY execute_transaction - amount exceeds limit
14:32:03  ⏸️  Awaiting approval: Financial transaction $15,000
```

**Files:**
- `resources/prompts/steering/cedar-policies.prompt.md` — NEW: Cedar generation prompt
- `src/services/steeringGenerationService.ts` — Add `generateCedarPolicies()` method
- `src/types/config.ts` — Add policy configuration schema
- `resources/scripts/setup.sh` — Add Step 2b (Policy Engine creation) ✅ DONE
- `resources/scripts/destroy.sh` — Add Step 1b (Policy Engine deletion) ✅ DONE
- `resources/prompts/steering/tech-steering.prompt.md` — Reference Cedar policies
- `resources/prompts/steering/agentify-integration-steering.prompt.md` — Add policy event patterns `L`
...
60. [ ] Demo Library Storage — Implement cloud storage for saving completed demos with metadata, tags, and search capability `L`

61. [ ] Demo Sharing — Add team sharing functionality with permissions and version tracking for collaborative demo development `M`

62. [ ] Demo Analytics — Build tracking for demo usage metrics: runs, customer reactions, conversion correlation `L`

63. [ ] Multi-Region Deployment — Add region selector and deployment automation for production deployments in us-east-1, us-west-2, eu-west-1 `M`

64. [ ] Demo Export — Create export functionality for packaging demos as standalone artifacts for offline or customer-site execution `M`
